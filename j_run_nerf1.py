...
from j_run_nerf_helpers import *  # 注意：这个文件也需要转换为Jittor版本

from load_llff import load_llff_data  # 使用转换后的load_llff.py
from load_deepvoxels import load_dv_data
from j_load_blender import load_blender_data
from load_LINEMOD import load_LINEMOD_data

# Jittor自动管理设备，不需要显式指定
jt.flags.use_cuda = 0  # 启用GPU
np.random.seed(0)
DEBUG = False
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        rgbs.append(rgb.numpy())  # Jittor使用.numpy()代替.cpu().numpy()
        ...
        if gt_imgs is not None and render_factor==0:
            p = -10. * np.log10(np.mean(np.square(rgb.numpy() - gt_imgs[i])))
            print(p)
        """
...
def create_nerf(args):
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        ckpt = jt.load(ckpt_path)  # Jittor使用jt.load
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    dists = jt.concat([dists, jt.array(np.array([1e10], dtype=np.float32)).expand(dists[...,:1].shape)], -1)  # [N_rays, N_samples]
...
def render_rays(ray_batch,
    ...
    if N_importance > 0:
        ...
        ret['acc0'] = acc_map_0
        ret['z_std'] = jt.std(z_samples, dim=-1)  # [N_rays]
...
def train():
    ...
    if args.render_only:
        print('RENDER ONLY')
        with jt.no_grad():  # Jittor使用jt.no_grad()
            if args.render_test:
                # render_test切换到测试位姿
                images = images[i_test]
            else:
                # 默认是更平滑的render_poses路径
                images = None
    ...
    if use_batching:
        ...
        rays_rgb = np.stack([rays_rgb[i] for i in i_train], 0) # 仅训练图像
    ...
    for i in trange(start, N_iters):
        ...
        # Jittor关键不同：将loss传入optimizer.step()
        optimizer.step(loss)
